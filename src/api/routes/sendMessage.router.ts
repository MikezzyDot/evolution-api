...
export class MessageRouter extends RouterBroker {
	constructor(...guards: RequestHandler[]) {
		...
		this.router
			...
			.post(this.routerPath('sendCustomMeta'), ...guards, async (req, res) => {
				logger.verbose('request received in sendCustomMeta');
				logger.verbose('request body: ');
				logger.verbose(req.body);

				logger.verbose('request query: ');
				logger.verbose(req.query);
				const response = await this.dataValidate<SendCustomMetaDto>({
					request: req,
					schema: customMetaMessageSchema,
					ClassRef: SendCustomMetaDto,
					execute: (instance, data) => sendMessageController.sendCustomMetaMessage(instance, data),
				});
